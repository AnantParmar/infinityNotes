...
const Noteitem = (props) => {
  const { title, description, updateNote, key, id, tag } = props;
  ...
  return (
    <div className="col-md-3 my-3 cardStyle" id={id} >
      <div className="card bg-dark h-100 text-white" key={key} >
        ...
        <span className="position-absolute top-0 start-100 badge rounded-pill bg-danger" style={{transform: "translate(-99%,-50%)"}}>
          ...
          <i className="fa-solid fa-pen-to-square mx-2" onClick={()=>{updateNote(id,title,description,tag)}}></i>
